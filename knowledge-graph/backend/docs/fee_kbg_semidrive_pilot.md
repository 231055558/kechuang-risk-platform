...
经用户确认后调用天眼查15个低价企业接口，芯驰科技分摊预计费用3.15元。主数据库
新增11条当前主要人员返回记录（合并同一人后为10个主体）、企业股东接口单页前
20名和疑似实际控制人记录。当前股东比例只用于主体重要性筛选，不能解释为上市
披露口径或控制权法律结论。诉讼、被执行、行政处罚、严重违法和经营异常接口均未
返回芯驰科技匹配记录，因此未凭空新增风险事件传导链。
