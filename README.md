...
数据来自企业官网、交易所披露、论文、公开技术文档以及团队提供的企业材料。它们是带截止日期的研究快照，不是实时抓取结果，也不代表对企业的监管认定或投资结论。

仓库同时保留团队授权公开的原始输入快照，位于 `data/raw/`，包括行业风险 SQLite 数据库、
数据说明和 7 家企业的 Excel、PDF、ZIP 材料。完整文件清单与校验方式见
`data/raw/README.md` 和 `data/raw/SHA256SUMS`。应用运行时读取 `src/data/` 下经过校验的派生 JSON，
不会直接修改原始文件。
...
- 更换浏览器或设备不会自动同步这些状态。
- 项目包含用于重建派生数据的 SQLite 文件，但运行时不依赖外部数据库服务；项目不包含用户账户、权限系统或跨设备同步服务。
