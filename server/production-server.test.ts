...
async function startTestServer(options?: {
  ...
  listRiskGraphPostgresCompanies?: () => unknown | Promise<unknown>
  ...
  const server = createProductionServer({
    ...
    listRiskGraphPostgresCompanies: options?.listRiskGraphPostgresCompanies,
...
test("risk graph API returns PostgreSQL-backed directory and directed views", async () => {
  ...
  const testServer = await startTestServer({
    ...
    listRiskGraphPostgresCompanies: () => ({
