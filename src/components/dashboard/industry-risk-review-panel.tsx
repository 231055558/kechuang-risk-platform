...
import {
  RiskOverviewPrototype,
  type RiskOverviewPrototypeVariant,
  useRiskOverviewPrototypeVariant,
} from "@/components/dashboard/industry-risk-overview.prototype"
...
export function IndustryRiskReviewPanel({
  ...
  const prototypeVariant = useRiskOverviewPrototypeVariant()
    ...
    <>
      <Reveal>
        <GlassPanel
          ...
          data-prototype={prototypeVariant ?? undefined}
        >
...
function IndustryRiskAssessmentContent({
  ...
  selectedRank: number
  prototypeVariant: RiskOverviewPrototypeVariant | null
  onNavigate?: (
    view: "reports" | "realtime" | "events" | "intelligence"
  ) => void
}) {
  ...
  return (
    <Tabs defaultValue="overview" className="industry-risk-detail-tabs">
      ...
      <TabsContent value="overview">
        {prototypeVariant ? (
          <RiskOverviewPrototype
            key={`${response.company.id}:${prototypeVariant}`}
            response={response}
            selectedRank={selectedRank}
            onNavigate={onNavigate}
          ...
          <>
            ...
            <section className="customer-risk-hero">
              ...
              >
                ...
                <p>
                  基于当前可用指标进行同业比较，数值越高表示相对风险越突出。
                </p>
                <div className="customer-risk-score-track" aria-hidden="true">
                  <span
                    style={{ width: `${assessment.totalRiskScore ?? 0}%` }}
                  />
                </div>
              </article>
