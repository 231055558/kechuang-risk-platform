...
test("the design37 fixture preserves the expanded official-report audit matrix", () => {
  ...
  assert.equal(design37Dataset.observations.length, 1_384)
  assert.equal(design37Dataset.coverage.length, 814)
  assert.equal(design37Dataset.sources.length, 119)
  assert.equal(design37Dataset.screeningHits.length, 19)
  assert.equal(design37Dataset.inquiryEvidence.length, 51)
  assert.equal(design37Dataset.litigationEvidence.length, 1)
  assert.equal(design37Dataset.supplementaryObservations.length, 205)
