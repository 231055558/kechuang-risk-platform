...
test("method and comparison controls live in their relevant content regions", () => {
  ...
  assert.match(formulaSource, /指标计算公式/)
  assert.doesNotMatch(formulaSource, /目标指标公式|当前运行公式|两者关系/)
  assert.match(formulaSource, /<math aria-label=/)
  assert.match(formulaSource, /<mfrac>/)
  assert.match(formulaSource, /\{spec\.runtime\}/)
  assert.doesNotMatch(formulaSource, /\{spec\.target\}/)
